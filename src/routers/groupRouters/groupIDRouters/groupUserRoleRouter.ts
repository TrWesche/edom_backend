...
// Add User Role
groupUserRoleRouter.post("/roles", authMW.defineGroupPermissions(["read_user_role", "create_user_role"]), authMW.validatePermissions, async (req, res, next) => {
    try {
        // Preflight
...
// Get User Roles
groupUserRoleRouter.get("/roles", authMW.defineGroupPermissions(["read_user_role"]), authMW.validatePermissions, async (req, res, next) => {
    try {
        // Preflight
...
// Remove User Role
groupUserRoleRouter.delete("/roles", authMW.defineGroupPermissions(["read_user_role", "delete_user_role"]), authMW.validatePermissions, async (req, res, next) => {
    try {
        // Preflight
        if (!req.user?.id || !req.targetUID || !req.groupID || !req.body.roleID) {
